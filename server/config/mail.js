...
module.exports = [
  {
    ...
    text: (info) =>
      `
        По <a style="color:#9509ef;" href="${process.env.CLIENT_URL}/${
        info.articleType
      }/${info.articleId}">${
        info.articleType === "offer" ? "предложению" : "заказу"
      } № ${info.articleId}</a> поступила новая заявка`,
  },
  {
    id: 2,
  ...
  },
  {
    ...
    text: (info) =>
      `
        По
        <a style="color:#9509ef;" href="${process.env.CLIENT_URL}/${
        info.articleType
      }/${info.articleId}"
        >
          ${info.articleType === "offer" ? "предложению" : "заказу"} № 
